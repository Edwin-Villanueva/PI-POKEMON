...
try {
    ...
    if( name ){//si paso 2 parametros con la misma variable me genera un array de valores
       ...
       pokemonsDB =await Pokemon.findAll({
        ...
        let PokesPromises=pokemonsAPI.data.results.map((poke)=>{//genero un array de promesas
            return axios.get(poke.url)
        })
        let Next20=await axios.get(pokemonsAPI.data.next);
        let OtherPokesPromises=Next20.data.results.map((poke)=>{//genero un array de promesas
            return axios.get(poke.url)
        })

        Promise.all([...PokesPromises,...OtherPokesPromises])
        .then(infoPoke=>{ 
            ...
            let pDB_filt =pokemonsDB.map((pokemon)=> { //puede haber mas de 1 propiedad pero solo mando 1
                return {
                    name:pokemon.name,
                    id:pokemon.id
                }
